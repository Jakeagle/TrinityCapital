...
class QuickTimeManager {
  ...
  startQuickTimeInterval(username) {
    ...
    // Check every 500ms to ensure we don't miss transactions
  ...
  async checkAndProcessTransactions(username) {
    ...
    try {
      ...
      // Check paychecks
      const paychecks = userProfile.checkingAccount?.paychecks || [];
      for (const paycheck of paychecks) {
        if (paycheck.nextExecution) {
          const nextExecTime = new Date(paycheck.nextExecution);
          const isDue =
            currentSimulatedTime.getTime() >= nextExecTime.getTime();

          if (isDue) {
            console.log(
              `⏱️  [QuickTime] Paycheck due for ${username}: ${paycheck.Name}`,
            );
            await this.processQuickTimeTransaction(
              username,
              paycheck,
  ...
  async processPendingTransactions(username) {
    ...
    try {
      ...
      // RESCHEDULE PAYCHECKS FOR QUICK TIME
      const paychecks = userProfile.checkingAccount?.paychecks || [];
      console.log(`💵 [QuickTime] Processing ${paychecks.length} paychecks...`);

      const rescheduledPaychecks = paychecks.map((paycheck) => {
        console.log(
          `  📊 Paycheck: "${paycheck.Name}", frequency: "${paycheck.interval || paycheck.frequency}", nextExecution: ${paycheck.nextExecution}`,
        );
        if (paycheck.nextExecution) {
          const originalNextExec = new Date(paycheck.nextExecution);
          const delayMs = originalNextExec.getTime() - now.getTime();
          const delayDays = delayMs / (24 * 60 * 60 * 1000);
          const quickTimeDelayMs = delayDays * 1000;
          const newNextExec = new Date(
          ...
          console.log(
            `  • Paycheck "${paycheck.Name}": ${delayDays.toFixed(1)} days → ${(quickTimeDelayMs / 1000).toFixed(1)}s`,
          );
          hasUpdates = true;

          return { ...paycheck, nextExecution: newNextExec.toISOString() };
        }
        return paycheck;
      });
          ...
          .updateOne(
            { "checkingAccount.accountHolder": username },
            {
              $set: {
                ...
                "checkingAccount.paychecks": rescheduledPaychecks,
              },
            },
          );
