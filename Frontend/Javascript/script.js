...
const socket = io("http://https://tcstudentserver-production.up.railway.app");
...
async function initializeStudentMessaging(studentName) {
  ...
  try {
    console.log(
      ...
      `http://https://tcstudentserver-production.up.railway.app/messages/${studentName}`,
    );
...
async function openMessageCenter() {
  ...
  if (newThreadButton && !newThreadButton.dataset.listenerAdded) {
    newThreadButton.addEventListener("click", async () => {
      try {
        ...
        const response = await fetch(
          `http://https://tcstudentserver-production.up.railway.app/classmates/${currentProfile.memberName}`,
        );
        if (!response.ok) {
  ...
  try {
    const response = await fetch(
      `http://https://tcstudentserver-production.up.railway.app/messages/${currentProfile.memberName}`,
    );
...
async function createNewThread(recipientId) {
  try {
    const response = await fetch("http://https://tcstudentserver-production.up.railway.app/newThread", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
/***********************************************************Server Functions**********************************************/
const testServerProfiles = "http://https://tcstudentserver-production.up.railway.app/profiles";

const loanURL = "http://https://tcstudentserver-production.up.railway.app/loans";

const donationURL = "http://https://tcstudentserver-production.up.railway.app/donations";

const donationSavingsURL = "http://https://tcstudentserver-production.up.railway.app/donationsSavings";

const balanceURL = "http://https://tcstudentserver-production.up.railway.app/initialBalance";
  ...
  try {
    ...
    if (currentProfile) {
      ...
      if (
        ...
        try {
          const cleanupUrl = `http://https://tcstudentserver-production.up.railway.app/sample/cleanup-student/${encodeURIComponent(
            currentProfile.memberName,
          )}`;
          console.log(
