...
async function sendBillData(type, amount, interval, name, cat, date) {
  try {
    ...
    // Record lesson progress for bill/payment creation
    if (typeof window.recordLessonAction === 'function') {
      ...
      // Determine which lesson conditions this satisfies
      if (type === 'bill') {
        // Bills can contribute to multiple lesson types
        window.recordLessonAction('spending_analyzed', actionDetails);
        window.recordLessonAction('expenses_categorized', actionDetails);
        window.recordLessonAction('budget_balanced', actionDetails);
        window.recordLessonAction('cost_comparison_completed', actionDetails);
        window.recordLessonAction('balance_sheet_created', actionDetails);
        window.recordLessonAction('paycheck_analyzed', actionDetails);

        console.log('📚 Recorded bill creation for lesson progress:', name);
      } else if (type === 'payment') {
        // Payments satisfy different conditions
        window.recordLessonAction('smart_goal_validated', actionDetails);
        window.recordLessonAction('budget_balanced', actionDetails);
        window.recordLessonAction('payment_methods_compared', actionDetails);

        console.log('📚 Recorded payment creation for lesson progress:', name);
      }
    }
